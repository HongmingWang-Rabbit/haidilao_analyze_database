...
class DatabaseConfig:
    ...
    def __init__(self, is_test: bool = False):
        ...
        if not all([self.host, self.user, self.password, self.database]):
            missing = []
            if not self.host:
                missing.append(f"{prefix}PG_HOST")
            if not self.user:
                missing.append(f"{prefix}PG_USER")
            if not self.password:
                missing.append(f"{prefix}PG_PASSWORD")
            if not self.database:
                missing.append(f"{prefix}PG_DATABASE")
...
class DatabaseSetup:
    ...
    def __init__(self, db_manager: DatabaseManager):
        ...
        self.sql_dir = Path(__file__).parent.parent / \
            "haidilao-database-querys"
    ...
    def verify_database_structure(self) -> bool:
        ...
        required_tables = ['store', 'time_segment', 'daily_report',
                           'store_time_report', 'store_monthly_target']
        ...
        try:
            ...
            stores = self.db_manager.fetch_all(
            ...
            time_segments = self.db_manager.fetch_all(
                "SELECT COUNT(*) as count FROM time_segment")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Database utilities for Haidilao system")
    parser.add_argument("--test", action="store_true",
                        help="Use test database")
    parser.add_argument("--setup", action="store_true", help="Setup database")
    parser.add_argument("--verify", action="store_true",
                        help="Verify database connection")
    parser.add_argument("--check-structure",
                        action="store_true", help="Check database structure")
