...
-- This addresses the issue where different sales modes (dine-in/takeout) are being overwritten

BEGIN;

-- Step 1: Add sales_mode column to dish_monthly_sale table
ALTER TABLE dish_monthly_sale 
ADD COLUMN sales_mode VARCHAR(10) DEFAULT 'dine-in' CHECK (sales_mode IN ('dine-in', 'takeout'));

-- Step 2: Update the comment for the table
COMMENT ON COLUMN dish_monthly_sale.sales_mode IS 'Sales mode: dine-in or takeout';
