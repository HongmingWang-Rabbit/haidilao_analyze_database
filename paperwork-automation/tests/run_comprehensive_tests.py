...
def discover_and_run_all_tests():
    ...
    test_modules = [
        ...
        'test_monthly_dishes_worksheet',
        'test_material_usage_summary_worksheet',
        'test_detailed_material_spending_worksheet',
        'test_beverage_summary_worksheet',
        'test_beverage_variance_worksheet',
        'test_daily_store_tracking_worksheet',
        'test_extract_all',
    ...
    for module_name in test_modules:
        ...
        try:
            ...
            print(
                f"   {status} - {module_tests} tests, {module_failures} failures, {module_errors} errors ({end_time - start_time:.2f}s)")
    ...
    if total_tests > 0:
        success_rate = ((total_tests - total_failures -
                        total_errors) / total_tests) * 100
    ...
    for module_name, result in results.items():
        ...
        print(
            f"{status_icon} {module_name:<35} {result['tests']:>3} tests  {result['time']:>6.2f}s")
    ...
    covered_modules = {
        ...
        'lib/monthly_dishes_worksheet.py': 'test_monthly_dishes_worksheet' in results,
        'lib/material_usage_summary_worksheet.py': 'test_material_usage_summary_worksheet' in results,
        'lib/beverage_summary_worksheet.py': 'test_beverage_summary_worksheet' in results,
        'lib/beverage_variance_worksheet.py': 'test_beverage_variance_worksheet' in results,
        'lib/daily_store_tracking_worksheet.py': 'test_daily_store_tracking_worksheet' in results,
    ...
    print(
        f"Module Coverage: {covered_count}/{total_modules} ({coverage_percentage:.1f}%)")
    print()
    ...
    uncovered_modules = [module for module,
                         covered in covered_modules.items() if not covered]
    if uncovered_modules:
        print(
            f"📝 Create tests for {len(uncovered_modules)} uncovered modules:")
    ...
    for test_file in test_files:
        ...
        if test_path.exists():
            try:
                ...
                print(
                    f"📄 {test_file:<40} {len(test_functions):>3} test functions")
                total_test_functions += len(test_functions)

                # Show some test function examples
                if test_functions:
                    print(
                        f"   Sample tests: {', '.join(test_functions[:3])}...")
        ...
        if test_path.exists():
            try:
                ...
                if 'success' in content or 'valid' in content:
                    categories['success_cases'] += content.count(
                        'success') + content.count('valid')
                if 'error' in content or 'failure' in content or 'exception' in content:
                    categories['error_handling'] += content.count(
                        'error') + content.count('failure')
                if 'edge' in content or 'empty' in content or 'none' in content:
                    categories['edge_cases'] += content.count(
                        'edge') + content.count('empty')
                if 'integration' in content or 'workflow' in content:
                    categories['integration'] += content.count(
                        'integration') + content.count('workflow')
                if 'validation' in content or 'validate' in content:
                    categories['data_validation'] += content.count(
                        'validation') + content.count('validate')
