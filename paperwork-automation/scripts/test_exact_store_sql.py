#!/usr/bin/env python3
"""
Test the exact SQL query from get_store_gross_profit_data method
"""
...
def test_exact_sql():
    """Test the exact SQL from the method"""
    
    config = DatabaseConfig()
    db_manager = DatabaseManager(config)
    
    # Parse target date (same as in method)
    target_date = "2025-05-01"
    target_dt = datetime.strptime(target_date, '%Y-%m-%d')
    current_year = target_dt.year
    current_month = target_dt.month

    # Calculate previous month and year (same as in method)
    if current_month == 1:
        prev_month = 12
    ...
    # Exact parameters from the method
    params = (
    ...
    try:
        ...
        print("Executing exact query from method...")
        ...
        if results:
            print("Results:")
            for row in results:
                print(f"  Store {row['store_id']}: Revenue=${row['current_revenue']:,.0f}, Cost=${row['current_cost']:,.0f}, Prev Rev=${row['previous_revenue']:,.0f}, Prev Cost=${row['previous_cost']:,.0f}")
        else:
            print("No results returned (this might be the issue)")
