...
class GrossMarginAnalysisSheet:
    ...
    def _get_dish_price_impact(self, store_id: int, current_month: Tuple, last_month: Tuple) -> Dict[str, float]:
        """Calculate impact of dish price changes on gross margin"""
    ...
    def _get_material_price_impact(self, store_id: int, current_month: Tuple, last_month: Tuple) -> Dict[str, float]:
        """Calculate impact of material price changes on gross margin"""
        current_year, current_mon = current_month
        last_year, last_mon = last_month
        ...
        query = """
            ...
            SELECT
                SUM(CASE
                    WHEN lm.material_id IS NOT NULL AND lm.last_price > 0
                    THEN (cm.current_price - lm.last_price) * cm.material_used
                    ELSE 0
                END) as price_impact
            FROM current_materials cm
            LEFT JOIN last_materials lm ON cm.material_id = lm.material_id
    ...
    def _calculate_dish_loss_impact(self, store_id: int, current_month: Tuple) -> float:
    ...
    def _get_store_analysis(self, store_id: int, store_name: str) -> Dict[str, Any]:
        """Get complete gross margin analysis for a store"""
        ...
        last_month_data = self._get_revenue_and_cost(
        ...
        last_month = (date_ranges['last_month'][0].year, date_ranges['last_month'][0].month)

        dish_price_impact = self._get_dish_price_impact(store_id, current_month, last_month)
        material_price_impact = self._get_material_price_impact(store_id, current_month, last_month)
        dish_loss_impact = self._calculate_dish_loss_impact(store_id, current_month)
        dish_loss_impact_last = self._calculate_dish_loss_impact(store_id, last_month)
        ...
        last_margin_decimal = last_month_data['gross_margin'] / 100
        ...
        if revenue > 0:
            ...
            discount_pct_last = last_month_data['discount'] / last_month_data['revenue'] if last_month_data['revenue'] > 0 else 0
            ...
            # Last month restored: =1-(1-D5)*V5/(V5/(1-M5)) where M5=last discount %
            if discount_pct_last < 1 and last_month_data['revenue'] > 0:
                last_discount_restored_margin = (1 - (1 - last_margin_decimal) * last_month_data['revenue'] /
                                                (last_month_data['revenue'] / (1 - discount_pct_last))) * 100
            else:
                last_discount_restored_margin = 0

            # Discount margin impact: the difference between the two restored margins
            # This shows the impact of discount change on margin
            discount_margin_impact = current_discount_restored_margin - last_discount_restored_margin
        ...
        return {
            ...
            'last_margin': last_month_data['gross_margin'],
            'margin_change': current_data['gross_margin'] - last_month_data['gross_margin'],
            ...
            'last_discount': last_month_data['discount'],
            'discount_change': ((current_data['discount'] - last_month_data['discount']) / last_month_data['discount'] * 100) if last_month_data['discount'] > 0 else 0,
            'current_discount_restored_margin': current_discount_restored_margin,
            'last_discount_restored_margin': last_discount_restored_margin,
    ...
    def generate_sheet(self, workbook):
        """Generate the gross margin analysis sheet"""
        ...
        subheaders = [
            '',  # Store name
            '本月(%)', '上月(%)', '环比(pp)',  # Gross margin
            '变动金额', '还原毛利率(%)', '毛利率影响(pp)',  # Dish price
            '变动金额', '还原毛利率(%)', '毛利率影响(pp)',  # Material price
            '本月', '上月', '环比(%)', '本月还原毛利率(%)', '上月还原毛利率(%)', '毛利率影响(pp)',  # Discount
        ...
        ws.merge_cells(start_row=1, start_column=21, end_row=2, end_column=21)  # Last revenue

        # Process each store
        row_num = 3
        for store_id, store_name in stores:
            logger.info(f"Processing gross margin analysis for {store_name}")
            ...
            try:
                ...
                ws.cell(row=row_num, column=3, value=round(data['last_margin'] / 100, 4)).border = border
                ...
                ws.cell(row=row_num, column=12, value=round(data['last_discount'], 2)).border = border
                ...
                ws.cell(row=row_num, column=15, value=round(data['last_discount_restored_margin'] / 100, 4)).border = border
                ...
                ws.cell(row=row_num, column=21, value=round(data['last_revenue'], 2)).border = border
