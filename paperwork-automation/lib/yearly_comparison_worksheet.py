...
class YearlyComparisonWorksheetGenerator:
    ...
    def generate_worksheet(self, wb, yearly_current, yearly_previous):
        ...
        total_current_tables = sum(float(row['total_tables']) for row in yearly_current)
        total_previous_tables = sum(float(row['total_tables']) for row in yearly_previous)
        total_current_revenue = sum(float(row['total_revenue']) for row in yearly_current)
        total_previous_revenue = sum(float(row['total_revenue']) for row in yearly_previous)
        total_current_turnover = sum(float(row['avg_turnover_rate']) for row in yearly_current) / len(yearly_current)
        total_previous_turnover = sum(float(row['avg_turnover_rate']) for row in yearly_previous) / len(yearly_previous) if yearly_previous else 0
        total_current_per_table = sum(float(row['avg_per_table']) for row in yearly_current) / len(yearly_current)
        total_previous_per_table = sum(float(row['avg_per_table']) for row in yearly_previous) / len(yearly_previous) if yearly_previous else 0
