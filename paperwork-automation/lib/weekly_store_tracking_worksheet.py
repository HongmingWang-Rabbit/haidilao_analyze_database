"""
Weekly Store Performance Tracking Worksheet Generator

This module generates a weekly store performance tracking worksheet (门店周-加拿大)
that aggregates and compares last 7 days performance vs same period previous year.

The worksheet includes:
- Table turnover rate comparison and scoring (7-day average)
- Weekly revenue comparison and scoring (7-day total)
...
class WeeklyStoreTrackingGenerator:
    ...
    def generate_worksheet(self, workbook: Workbook, target_date: str) -> Worksheet:
        """
        Generate weekly store performance tracking worksheet.
        ...
        try:
            self.logger.info(
                f"Generating weekly store tracking worksheet for week ending {target_date}")

            # Create worksheet
            ws = workbook.create_sheet("门店周-加拿大")
            ...
            # Get store data for the 7-day period
            store_data = self._get_store_performance_data(start_dt, target_dt)

            # Add regional summary (row 3)
            self._add_regional_summary(ws, store_data, start_dt, target_dt, prev_year_start_dt, prev_year_end_dt)

            # Add store data (rows 4+)
            self._add_store_data(ws, store_data, start_dt, target_dt, prev_year_start_dt, prev_year_end_dt)

            # Apply formatting
            self._apply_formatting(ws, len(store_data))

            self.logger.info(
                f"Weekly store tracking worksheet generated with {len(store_data)} stores")

            return ws

        except Exception as e:
            self.logger.error(
                f"Error generating weekly store tracking worksheet: {str(e)}")
            raise

    def _generate_headers(self, ws: Worksheet, start_dt: datetime, target_dt: datetime, 
                         prev_year_start_dt: datetime, prev_year_end_dt: datetime) -> None:
        ...
        headers_row1 = [
            ...
            "周翻台率-考核",   # F1 - Weekly Table Turnover Rate Assessment
            "",              # G1 - Empty
            "",              # H1 - Empty
            "",              # I1 - Empty
        ...
        prev_period = f"24年{prev_year_start_dt.month}月{prev_year_start_dt.day}日-{prev_year_end_dt.month}月{prev_year_end_dt.day}日"
    ...
    def _get_store_performance_data(self, start_dt: datetime, target_dt: datetime) -> List[Dict[str, Any]]:
        """
        Get store performance data for the 7-day period.

        Args:
            start_dt: Start date of the 7-day period
            target_dt: End date of the 7-day period

        Returns:
            List of store performance data dictionaries with weekly aggregations
        """
        try:
            # Get weekly store performance data (aggregated for 7 days)
            store_data = self.data_provider.get_weekly_store_performance(
                start_dt.strftime('%Y-%m-%d'), target_dt.strftime('%Y-%m-%d'))

            if not store_data:
                self.logger.warning(
                    "⚠️ No weekly store performance data found, using mock data")
                return self._get_mock_weekly_store_data()

            self.logger.info(
                f"✅ Retrieved {len(store_data)} stores with real weekly database data")

            # Validate that we have the required fields
            for store in store_data:
                required_fields = ['store_id', 'store_name', 'manager_name', 'seating_capacity',
                                   'current_avg_turnover_rate', 'current_total_revenue',
                                   'prev_avg_turnover_rate', 'prev_total_revenue']
            ...
            self.logger.error(
                f"❌ Error getting weekly store performance data: {str(e)}")
    ...
    def _add_regional_summary(self, ws: Worksheet, store_data: List[Dict[str, Any]],
        ...
        if store_data:
            ...
            total_current_tables = sum(
                s['current_avg_turnover_rate'] * s['seating_capacity'] for s in store_data)
            total_prev_tables = sum(
                s['prev_avg_turnover_rate'] * s['seating_capacity'] for s in store_data)
            ...
            sum_current_revenue = sum(s['current_total_revenue'] for s in store_data)
            sum_prev_revenue = sum(s['prev_total_revenue'] for s in store_data)
        else:
    ...
    def _calculate_comprehensive_scores(self, store_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        current_turnovers = [float(s['current_avg_turnover_rate'])
                             for s in store_data]
        prev_turnovers = [float(s['prev_avg_turnover_rate']) for s in store_data]
        current_revenues = [float(s['current_total_revenue']) for s in store_data]
        prev_revenues = [float(s['prev_total_revenue']) for s in store_data]
    ...
    def _add_store_data(self, ws: Worksheet, store_data: List[Dict[str, Any]],
        ...
        for i, store in enumerate(sorted_store_data):
            ...
            # Table turnover rate data (weekly averages)
            # F - Current week average turnover
            ws.cell(row=row, column=6, value=store['current_avg_turnover_rate'])
            # G - Previous year week average turnover
            ws.cell(row=row, column=7, value=store['prev_avg_turnover_rate'])
    ...
    def _apply_formatting(self, ws: Worksheet, num_stores: int) -> None:
        ...
        # Set column widths (same as daily version)
        column_widths = {
            'A': 6,   # Serial number
            'B': 15,  # Store name
            'C': 10,  # Manager name
        ...
        self.logger.info(
            "Formatting applied to weekly store tracking worksheet")