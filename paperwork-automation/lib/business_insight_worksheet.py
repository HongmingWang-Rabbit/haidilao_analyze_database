...
class BusinessInsightWorksheetGenerator:
    ...
    def generate_worksheet(self, wb, daily_data, monthly_data, previous_month_data, 
        ...
        daily_dict = {row['store_id']: row for row in daily_data} if isinstance(daily_data, list) else daily_data
        monthly_dict = {row['store_id']: row for row in monthly_data} if isinstance(monthly_data, list) else monthly_data
        prev_month_dict = {row['store_id']: row for row in previous_month_data} if isinstance(previous_month_data, list) else previous_month_data
        current_mtd_dict = {row['store_id']: row for row in current_mtd} if isinstance(current_mtd, list) else current_mtd
        prev_mtd_dict = {row['store_id']: row for row in prev_mtd} if isinstance(prev_mtd, list) else prev_mtd
        
        # Set column widths
        column_widths = [15, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12]
        ...
        current_row = self.add_store_basic_info_section(ws, current_row, daily_dict, monthly_dict, target_dt)
        
        # Section 2: 营业数据分析 (Business Data Analysis)
        current_row = self.add_business_analysis_section(ws, current_row, daily_dict, monthly_dict, prev_month_dict, current_mtd_dict, prev_mtd_dict, target_dt)
        
        # Section 3: 翻台率分析 (Turnover Rate Analysis)
        current_row = self.add_turnover_analysis_section(ws, current_row, daily_dict, monthly_dict, target_dt)
    ...
    def add_store_basic_info_section(self, ws, start_row, daily_dict, monthly_dict, target_dt):
        ...
        ws[f'A{current_row}'].fill = PatternFill(start_color="FFD700", end_color="FFD700", fill_type="solid")
        current_row += 1
        ...
        for store_id, store_name in self.store_names.items():
            ...
            daily_revenue = float(daily_row.get('revenue_tax_not_included', 0)) / 10000
            daily_tables = float(daily_row.get('tables_served_validated', 0))
            daily_customers = float(daily_row.get('customers', 0))
            daily_per_capita = daily_revenue * 10000 / daily_customers if daily_customers > 0 else 0
            daily_per_table = daily_revenue * 10000 / daily_tables if daily_tables > 0 else 0
            daily_takeout = float(daily_row.get('takeout_tables', 0))
            daily_excluded = float(daily_row.get('tables_served', 0)) - daily_tables
            
            # Check if store has no daily business (all zeros)
            store_closed_today = (daily_revenue == 0 and daily_tables == 0 and daily_customers == 0)
            
            # Calculate monthly values (MTD)
            monthly_revenue = float(monthly_row.get('monthly_revenue', 0)) / 10000 if monthly_row else 0
            monthly_tables = float(monthly_row.get('monthly_tables_validated', 0)) if monthly_row else 0
            monthly_customers = float(monthly_row.get('customers', 0)) if monthly_row else 0
            monthly_per_capita = monthly_revenue * 10000 / monthly_customers if monthly_customers > 0 else 0
            monthly_per_table = monthly_revenue * 10000 / monthly_tables if monthly_tables > 0 else 0
            monthly_takeout = float(monthly_row.get('takeout_tables', 0)) if monthly_row else 0
            monthly_excluded = float(monthly_row.get('tables_served', 0)) - monthly_tables if monthly_row else 0
            
            # Calculate previous month values (use prev_monthly_* fields from previous_month_data)
            prev_revenue = float(monthly_row.get('prev_monthly_revenue', 0)) / 10000 if monthly_row else 0
            prev_tables = float(monthly_row.get('prev_monthly_tables_validated', 0)) if monthly_row else 0
            prev_customers = float(monthly_row.get('prev_monthly_revenue', 0)) / float(monthly_row.get('prev_month_avg_per_table', 1)) if monthly_row and monthly_row.get('prev_month_avg_per_table', 0) > 0 else 0
            prev_per_capita = prev_revenue * 10000 / prev_customers if prev_customers > 0 else 0
            prev_per_table = float(monthly_row.get('prev_month_avg_per_table', 0)) if monthly_row else 0
            ...
            tables_change = ((monthly_tables - prev_tables) / prev_tables * 100) if prev_tables > 0 else 0
            per_capita_change = ((monthly_per_capita - prev_per_capita) / prev_per_capita * 100) if prev_per_capita > 0 else 0
        ...
        total_daily_revenue = sum(float(row['revenue_tax_not_included']) for row in active_daily_stores.values()) / 10000
        total_daily_tables = sum(float(row['tables_served_validated']) for row in active_daily_stores.values())
        total_daily_customers = sum(float(row['customers']) for row in active_daily_stores.values())
        total_daily_per_capita = total_daily_revenue * 10000 / total_daily_customers if total_daily_customers > 0 else 0
        total_daily_per_table = total_daily_revenue * 10000 / total_daily_tables if total_daily_tables > 0 else 0
        total_daily_takeout = sum(float(row['takeout_tables']) for row in active_daily_stores.values())
        total_daily_excluded = sum(float(row['tables_served']) - float(row['tables_served_validated']) for row in active_daily_stores.values())
        
        # Monthly totals (include all stores)
        total_monthly_revenue = sum(float(row['monthly_revenue']) for row in monthly_dict.values()) / 10000
        total_monthly_tables = sum(float(row['monthly_tables_validated']) for row in monthly_dict.values())
        total_monthly_customers = sum(float(row['customers']) for row in monthly_dict.values())
        total_monthly_per_capita = total_monthly_revenue * 10000 / total_monthly_customers if total_monthly_customers > 0 else 0
        total_monthly_per_table = total_monthly_revenue * 10000 / total_monthly_tables if total_monthly_tables > 0 else 0
        total_monthly_takeout = sum(float(row['takeout_tables']) for row in monthly_dict.values())
        total_monthly_excluded = sum(float(row['tables_served']) - float(row['monthly_tables_validated']) for row in monthly_dict.values())
        
        # Previous month totals
        total_prev_revenue = sum(float(row['prev_monthly_revenue']) for row in monthly_dict.values()) / 10000
        total_prev_tables = sum(float(row['prev_monthly_tables_validated']) for row in monthly_dict.values())
        total_prev_customers = total_prev_revenue * 10000 / (sum(float(row['prev_month_avg_per_table']) for row in monthly_dict.values() if row.get('prev_month_avg_per_table', 0) > 0) / len([row for row in monthly_dict.values() if row.get('prev_month_avg_per_table', 0) > 0])) if any(row.get('prev_month_avg_per_table', 0) > 0 for row in monthly_dict.values()) else 0
        total_prev_per_capita = total_prev_revenue * 10000 / total_prev_customers if total_prev_customers > 0 else 0
        total_prev_per_table = sum(float(row['prev_month_avg_per_table']) for row in monthly_dict.values() if row.get('prev_month_avg_per_table', 0) > 0) / len([row for row in monthly_dict.values() if row.get('prev_month_avg_per_table', 0) > 0]) if any(row.get('prev_month_avg_per_table', 0) > 0 for row in monthly_dict.values()) else 0
        
        # Comparison totals
        total_revenue_change = ((total_monthly_revenue - total_prev_revenue) / total_prev_revenue * 100) if total_prev_revenue > 0 else 0
        total_tables_change = ((total_monthly_tables - total_prev_tables) / total_prev_tables * 100) if total_prev_tables > 0 else 0
        total_per_capita_change = ((total_monthly_per_capita - total_prev_per_capita) / total_prev_per_capita * 100) if total_prev_per_capita > 0 else 0
        ...
        for col, value in enumerate(totals_data[1:], 2):  # Skip first column (store name)
            cell = ws.cell(row=current_row, column=col, value=value)
    ...
    def add_business_analysis_section(self, ws, start_row, daily_dict, monthly_dict, prev_month_dict, current_mtd_dict, prev_mtd_dict, target_dt):
        ...
        ws[f'A{current_row}'].alignment = Alignment(horizontal='center', vertical='center')
        ws[f'A{current_row}'].fill = PatternFill(start_color="FFE6CC", end_color="FFE6CC", fill_type="solid")
        current_row += 1
        ...
        for store_id, store_name in self.store_names.items():
            ...
            current_revenue = float(monthly_row.get('monthly_revenue', 0)) / 10000
            current_tables = float(monthly_row.get('monthly_tables_validated', 0))
            current_customers = float(monthly_row.get('customers', 0))
            
            # Previous month MTD data (use prev_mtd data)
            prev_revenue = float(prev_mtd_row.get('prev_mtd_revenue', 0)) / 10000 if prev_mtd_row else 0
            prev_tables = float(prev_mtd_row.get('prev_mtd_tables', 0)) if prev_mtd_row else 0
            prev_customers = float(prev_mtd_row.get('customers', 0)) if prev_mtd_row else 0
            
            # Calculate per capita and per table
            current_per_capita = current_revenue * 10000 / current_customers if current_customers > 0 else 0
            prev_per_capita = prev_revenue * 10000 / prev_customers if prev_customers > 0 else 0
            current_per_table = current_revenue * 10000 / current_tables if current_tables > 0 else 0
            prev_per_table = prev_revenue * 10000 / prev_tables if prev_tables > 0 else 0
            
            # Calculate growth rates
            revenue_growth = ((current_revenue - prev_revenue) / prev_revenue * 100) if prev_revenue > 0 else 0
            tables_growth = ((current_tables - prev_tables) / prev_tables * 100) if prev_tables > 0 else 0
            per_capita_growth = ((current_per_capita - prev_per_capita) / prev_per_capita * 100) if prev_per_capita > 0 else 0
            per_table_growth = ((current_per_table - prev_per_table) / prev_per_table * 100) if prev_per_table > 0 else 0
            
            # Check if store has minimal business (very low revenue and tables)
            store_has_minimal_business = current_revenue < 5.0 and current_tables < 10.0  # Less than 5万 revenue and 10 tables
    ...
    def add_turnover_analysis_section(self, ws, start_row, daily_dict, monthly_dict, target_dt):
        ...
        for store_id, store_name in self.store_names.items():
            ...
            monthly_turnover = float(monthly_row.get('avg_turnover_rate', 0)) if monthly_row else 0
            ...
            target_achievement = (monthly_turnover / target_turnover * 100) if target_turnover > 0 else 0
        ...
        for rank, data in enumerate(turnover_data, 1):
            if data['has_minimal_business']:
                row_data = [
                    ...
                    "无营业",
                    "停业",
                    round(data['prev_monthly_turnover'], 2) if data['prev_monthly_turnover'] > 0 else "无数据",
                    "无法计算",
