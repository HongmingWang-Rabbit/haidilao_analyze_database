#!/usr/bin/env python3
"""
Year-over-year comparison worksheet generator (对比上年表).
Only handles worksheet creation - receives data from main report generator.
This is similar to the monthly comparison but compares to the same month in the previous year.
"""
...
class YearlyComparisonDailyWorksheetGenerator:
    """Generate year-over-year comparison worksheet (对比上年表) from provided data"""
    ...
    def get_cell_value(self, content, category, store_name, col, daily_dict, monthly_dict,
                       prev_year_dict, targets_dict, current_mtd_dict, prev_year_mtd_dict,
        ...
        if store_name == "加拿大片区":
            ...
            prev_year_tables = sum(float(row['prev_yearly_tables'])
                                   for row in prev_year_dict.values()) if prev_year_dict else 0
            prev_year_tables_validated = sum(float(
                row['prev_yearly_tables_validated']) for row in prev_year_dict.values()) if prev_year_dict else 0
            prev_year_revenue = sum(float(row['prev_yearly_revenue'])
                                    for row in prev_year_dict.values()) if prev_year_dict else 0
            ...
            # Previous year MTD totals (for proper comparison)
            prev_year_mtd_tables = sum(float(
                row['prev_yearly_mtd_tables']) for row in prev_year_mtd_dict.values()) if prev_year_mtd_dict else 0
            prev_year_mtd_tables_validated = sum(float(
                row['prev_yearly_mtd_tables_validated']) for row in prev_year_mtd_dict.values()) if prev_year_mtd_dict else 0
            prev_year_mtd_revenue = sum(float(
                row['prev_yearly_mtd_revenue']) for row in prev_year_mtd_dict.values()) if prev_year_mtd_dict else 0
            ...
            prev_year_row = prev_year_dict.get(store_id, {})
            target_row = targets_dict.get(store_id, {})
            current_mtd_row = current_mtd_dict.get(store_id, {})
            prev_year_mtd_row = prev_year_mtd_dict.get(store_id, {})
            ...
            # Previous year data
            prev_year_tables = float(prev_year_row.get(
                'prev_yearly_tables', 0)) if prev_year_row else 0
            prev_year_tables_validated = float(prev_year_row.get(
                'prev_yearly_tables_validated', 0)) if prev_year_row else 0
            prev_year_revenue = float(prev_year_row.get(
                'prev_yearly_revenue', 0)) if prev_year_row else 0
            ...
            # Previous year MTD data (for proper comparison)
            prev_year_mtd_revenue = safe_float(prev_year_mtd_row.get(
                'prev_yearly_mtd_revenue', 0)) if prev_year_mtd_row else 0.0
            prev_year_mtd_tables = safe_float(prev_year_mtd_row.get(
                'prev_yearly_mtd_tables', 0)) if prev_year_mtd_row else 0.0
            prev_year_mtd_tables_validated = safe_float(prev_year_mtd_row.get(
                'prev_yearly_mtd_tables_validated', 0)) if prev_year_mtd_row else 0.0
        ...
        # Year-over-year comparisons: 去年同期总桌数 should be MTD, not full year
        table_change = monthly_tables_validated - prev_year_mtd_tables_validated

        # 去年截止目前营业收入(万) should be MTD
        revenue_change = current_mtd_revenue - prev_year_mtd_revenue
        current_mtd_avg_table = current_mtd_revenue / \
            current_mtd_tables if current_mtd_tables > 0 else 0

        # 去年单桌消费 should be average of whole year using total tables served
        prev_year_avg_table = prev_year_revenue / \
            prev_year_tables if prev_year_tables > 0 else 0
        avg_table_change = current_mtd_avg_table - prev_year_avg_table
        ...
        if content == "今日总桌数":
        ...
        elif content == "上年同期总桌数":
            return round(prev_year_mtd_tables_validated, 2)
        elif content == "对比上年同期总桌数":
        ...
        elif content == "上年截止目前营业收入(万)":
            return round(prev_year_mtd_revenue/10000, 2)
        elif content == "同比营业收入变化(万)":
            ...
            return round(prev_year_avg_table, 2)
        elif content == "同比上年变化":
            ...
            if store_name == "加拿大片区":
                # Calculate accurate weighted average to avoid precision loss
                if monthly_dict:
    ...
    def generate_worksheet(self, wb, daily_data, monthly_data, prev_year_data,
                           monthly_targets, current_mtd, prev_year_mtd,
                           daily_ranking, monthly_ranking, daily_ranking_values, monthly_ranking_values):
        """Generate the year-over-year comparison worksheet from provided data"""

        ws = wb.create_sheet("对比上年表")
        ...
        prev_year_dict = {row['store_id']: row for row in prev_year_data}
        targets_dict = {row['store_id']: row for row in monthly_targets}
        current_mtd_dict = {row['store_id']: row for row in current_mtd}
        prev_year_mtd_dict = {row['store_id']: row for row in prev_year_mtd}
        ...
        for category, content, color in data_rows:
            ...
            for col, store_name in enumerate(list(self.store_names.values()) + ["加拿大片区"], 3):
                value = self.get_cell_value(content, category, store_name, col,
                                            daily_dict, monthly_dict, prev_year_dict, targets_dict,
                                            current_mtd_dict, prev_year_mtd_dict, daily_ranking, monthly_ranking,
