...
class HiBowlDailyProcessor:
    ...
    def __init__(self):
        ...
        self.blue_fill = PatternFill(start_color="FF5B9BD5", end_color="FF5B9BD5", fill_type="solid")
    ...
    def process_daily_file(self, input_file: str, output_file: str, target_month: str = None) -> bool:
        ...
        try:
            ...
            summary = self._calculate_summary(daily_data, target_month, sheet_count)
            
            # Write to output file
            success = self._write_to_template(output_file, summary)
            
            if success:
                logger.info(f"Successfully generated Hi-Bowl report: {output_file}")
    ...
    def _read_daily_data(self, input_file: str) -> Optional[Tuple[pd.DataFrame, int]]:
        ...
        try:
            ...
            for sheet_name in xl_file.sheet_names:
                ...
                df_raw = safe_read_excel(input_file, sheet_name=sheet_name, header=None)
                ...
                df = safe_read_excel(input_file, sheet_name=sheet_name, skiprows=header_row)
                ...
                try:
                    ...
                    logger.warning(f"Could not parse date from sheet name: {sheet_name}")
                    continue
            ...
            logger.info(f"Loaded {len(combined_df)} total transactions from {valid_sheet_count} sheets")
    ...
    def _calculate_summary(self, df: pd.DataFrame, target_month: str = None, sheet_count: int = 0) -> Dict:
        ...
        summary['total_discount_without_tax'] = discount_total / 1.15  # Remove 15% tax
        ...
        summary['weekday_orders'] = len(df[weekday_mask])
        summary['weekend_orders'] = len(df[~weekday_mask])
        ...
        logger.info(f"Total orders: {summary['total_orders']}, Total guests: {summary['total_guests']}")
        logger.info(f"Weekday days: {summary['weekday_days']}, Weekend days: {summary['weekend_days']}")
    ...
    def _write_to_template(self, output_file: str, summary: Dict) -> bool:
        ...
        try:
            ...
            ws.cell(row=data_row, column=11).value = summary.get('total_discount_with_tax', 0)
            
            # Column L (12): 优惠总金额（不含税）
            ws.cell(row=data_row, column=12).value = summary.get('total_discount_without_tax', 0)
            
            # Column M (13): 小费-仅美国面馆/美国快餐填
            ws.cell(row=data_row, column=13).value = summary.get('total_tips', 0)
            
            # Column N (14): 实际营业天数
            ws.cell(row=data_row, column=14).value = summary.get('actual_operating_days', 0)
            
            # Column O (15): 应营业天数
            ws.cell(row=data_row, column=15).value = summary.get('scheduled_operating_days', 0)
            
            # Column P (16): 工作日天数
            ws.cell(row=data_row, column=16).value = summary.get('weekday_days', 0)
            
            # Column Q (17): 节假日天数
            ws.cell(row=data_row, column=17).value = summary.get('weekend_days', 0)
            
            # Column W (23): 全月就餐人数
            ws.cell(row=data_row, column=23).value = summary.get('total_guests', 0)
            
            # Column X (24): 工作日就餐人数
            ws.cell(row=data_row, column=24).value = summary.get('weekday_guests', 0)
            
            # Column Y (25): 节假日就餐人数
            ws.cell(row=data_row, column=25).value = summary.get('weekend_guests', 0)
            
            # Column AA (27): 全月订单数量-美国面馆/马来清真火锅/美国快餐
            ws.cell(row=data_row, column=27).value = summary.get('total_orders', 0)
            
            # Column AB (28): 工作日订单数-美国面馆/马来清真火锅/美国快餐
            ws.cell(row=data_row, column=28).value = summary.get('weekday_orders', 0)
            
            # Column AC (29): 节假日订单数-美国面馆/马来清真火锅/美国快餐
            ws.cell(row=data_row, column=29).value = summary.get('weekend_orders', 0)
            
            # Time segment revenue (columns AS-AV, 45-48)
            # Column AS (45): 08:00-13:59销售收入-新加坡面馆&马来清真火锅
            ws.cell(row=data_row, column=45).value = summary.get('revenue_08:00-13:59', 0)
            
            # Column AT (46): 14:00-16:59销售收入-新加坡面馆&马来清真火锅
            ws.cell(row=data_row, column=46).value = summary.get('revenue_14:00-16:59', 0)
            
            # Column AU (47): 17:00-21:59销售收入-新加坡面馆&马来清真火锅
            ws.cell(row=data_row, column=47).value = summary.get('revenue_17:00-21:59', 0)
            
            # Column AV (48): 22:00-(次)07:59销售收入-新加坡面馆&马来清真火锅
            ws.cell(row=data_row, column=48).value = summary.get('revenue_22:00-07:59', 0)
            
            # Time segment orders (columns BC-BF, 55-58)
            # Column BC (55): 08:00-13:59订单数-美国面馆&马来清真火锅
            ws.cell(row=data_row, column=55).value = summary.get('orders_08:00-13:59', 0)
            
            # Column BD (56): 14:00-16:59订单数-美国面馆&马来清真火锅
            ws.cell(row=data_row, column=56).value = summary.get('orders_14:00-16:59', 0)
            
            # Column BE (57): 17:00-21:59订单数-美国面馆&马来清真火锅
            ws.cell(row=data_row, column=57).value = summary.get('orders_17:00-21:59', 0)
            
            # Column BF (58): 22:00-(次)07:59订单数-美国面馆&马来清真火锅
            ws.cell(row=data_row, column=58).value = summary.get('orders_22:00-07:59', 0)
            
            # Apply blue fill to all cells that were filled
            cells_to_fill = [11, 12, 13, 14, 15, 16, 17, 23, 24, 25, 27, 28, 29, 45, 46, 47, 48, 55, 56, 57, 58]
