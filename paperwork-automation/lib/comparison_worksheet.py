...
class ComparisonWorksheetGenerator:
    ...
    def get_cell_value(self, content, category, store_name, col, daily_dict, monthly_dict, 
        ...
        if store_name == "加拿大片区":
            ...
            revenue = sum(float(row['revenue_tax_not_included']) for row in daily_dict.values())
            ...
            prev_mtd_tables = sum(float(row['prev_mtd_tables']) for row in prev_mtd_dict.values()) if prev_mtd_dict else 0
            prev_mtd_tables_validated = sum(float(row['prev_monthly_tables_validated']) for row in prev_mtd_dict.values()) if prev_mtd_dict else 0
            ...
            revenue = float(daily_row.get('revenue_tax_not_included', 0))
            ...
            prev_mtd_revenue = float(prev_mtd_row.get('prev_mtd_revenue', 0)) if prev_mtd_row else 0
            prev_mtd_tables = float(prev_mtd_row.get('prev_mtd_tables', 0)) if prev_mtd_row else 0
            prev_mtd_tables_validated = float(prev_mtd_row.get('prev_monthly_tables_validated', 0)) if prev_mtd_row else 0
 
            # Target data
            target_revenue = float(target_row.get('target_revenue', 100000)) if target_row else 100000
        ...
        prev_month_avg_table = prev_month_revenue / prev_month_tables if prev_month_tables > 0 else 0
